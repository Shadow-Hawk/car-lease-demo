...
const PATIENT_INDEX = "All_Patients"
const Encounter_INDEX = "All_Encounters"
...
type Encounter struct {
	...
	Date          string      `json:"date"` // epoch time
	HospitalName  string      `json:"hospital_name"`
	Department    string      `json:"department"`
	Physician     string      `json:"physician"`
	Patient       string      `json:"patient"`
	DiagnosisHash []byte      `json:"diagnosis_hash"`
	RxHash        []byte      `json:"rx_hash"`
}
...
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	var patientIndex AllPatients
	patientIndexBytes, err := json.Marshal(patientIndex)
	if err != nil {
		return nil, errors.New("Error creating AllPatients record")
	}
	err = stub.PutState(PATIENT_INDEX, patientIndexBytes)

	var encounterIndex AllEncounters
	encounterIndexBytes, err := json.Marshal(encounterIndex)
	if err != nil {
		return nil, errors.New("Error creating AllEncounters record")
	}
	err = stub.PutState(Encounter_INDEX, encounterIndexBytes)
...
func (t *SimpleChaincode) retrieve_v5c(stub shim.ChaincodeStubInterface, v5cID string) (Vehicle, error) {
	...
	if err != nil {
		fmt.Printf("RETRIEVE_V5C: Failed to invoke vehicle_code: %s", err); return v, errors.New("RETRIEVE_V5C: Error retrieving vehicle with v5cID = " + v5cID)
	}
...
func (t *SimpleChaincode) get_encounter(stub shim.ChaincodeStubInterface, encounterID string) (Encounter, error) {
	var e Encounter

	bytes, err := stub.GetState(encounterID);
	if err != nil {
		fmt.Printf("get_encounter: Failed to invoke get_encounter: %s", err); return e, errors.New("get_encounter: Error retrieving encounter with encounterID = " + encounterID)
	}

	err = json.Unmarshal(bytes, &e)
	if err != nil {
		fmt.Printf("get_encounter: Corrupt Encounter record " + string(bytes) + ": %s", err); return e, errors.New("get_encounter: Corrupt encounter record" + string(bytes))
	}

	return e, nil
}

func (t *SimpleChaincode) get_patient(stub shim.ChaincodeStubInterface, patientID string) (Patient, error) {
	var p Patient

	bytes, err := stub.GetState(patientID);
	if err != nil {
		fmt.Printf("get_patient: Failed to invoke get_patient: %s", err); return p, errors.New("get_patient: Error retrieving patient with patientID = " + patientID)
	}

	err = json.Unmarshal(bytes, &p)
	if err != nil {
		fmt.Printf("get_patient: Corrupt Patient record " + string(bytes) + ": %s", err); return p, errors.New("get_patient: Corrupt patient record" + string(bytes))
	}
...
func (t *SimpleChaincode) saveById(stub shim.ChaincodeStubInterface, v interface{}, id string) (bool, error) {
	bytes, err := json.Marshal(v)
	if err != nil {
		fmt.Printf("saveById: Error marshaling record: %s", err); return false, errors.New("Error marshaling record")
	}

	err = stub.PutState(id, bytes)
	if err != nil {
		fmt.Printf("saveById: Error storing record: %s", err); return false, errors.New("Error storing record")
	}
...
func (t *SimpleChaincode) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	caller, caller_affiliation, err := t.get_caller_data(stub)
	if err != nil {
		fmt.Printf("QUERY: Error retrieving caller details", err); return nil, errors.New("QUERY: Error retrieving caller details: " + err.Error())
	}
...
func (t *SimpleChaincode) create_vehicle(stub shim.ChaincodeStubInterface, caller string, caller_affiliation string, v5cID string) ([]byte, error) {
	...
	if err != nil {
		fmt.Printf("CREATE_VEHICLE: Invalid v5cID: %s", err); return nil, errors.New("Invalid v5cID")
	}
...
func (t *SimpleChaincode) create_encounter(stub shim.ChaincodeStubInterface, caller string, caller_affiliation string, encounterID string, date string, hospital string, department string, physician string, patient string) ([]byte, error) {
	v := Encounter{
	...
	record, err := stub.GetState(v.EncounterID)

	if record != nil {
		return nil, errors.New("Encounter already exists")
	}
	...
	// Save encounter
	_, err = t.saveById(stub, v, v.EncounterID)

	if err != nil {
		fmt.Printf("create_encounter: Error saving changes: %s", err); return nil, errors.New("Error saving changes")
	}


	// Update encounter index
	bytes, err := stub.GetState(Encounter_INDEX)

	if err != nil {
		return nil, errors.New("Unable to get AllEncounters")
	}

	var allEncounters AllEncounters

	err = json.Unmarshal(bytes, &allEncounters)

	if err != nil {
		return nil, errors.New("Corrupt AllEncounters record")
	}

	allEncounters.Encounters = append(allEncounters.Encounters, encounterID)

	_, err = t.saveById(stub, allEncounters, Encounter_INDEX)

	if err != nil {
		fmt.Printf("create_encounter: Error saving Encounter_INDEX: %s", err); return nil, errors.New("Error saving Encounter_INDEX")
	}

	/*bytes, err = json.Marshal(allEncounters)

	if err != nil {
		fmt.Print("Error creating AllEncounters record")
	}

	err = stub.PutState(Encounter_INDEX, bytes)
	...
	p, err := t.get_patient(stub, patient)

	if err == nil {
		// Patient exists, append this encounter
		p.EncounterIDs = append(p.EncounterIDs, encounterID)
	} else {
		// insert patient
		p = Patient{
			ID : patient,
			EncounterIDs : append([]string{}, encounterID),
		}
	}
	_, err = t.saveById(stub, p, p.ID)
	if err != nil {
		fmt.Printf("create_encounter: Error saving patient: %s", err); return nil, errors.New("Error saving patient")
	}
	...
	//TODO
	bytes, err = stub.GetState(PATIENT_INDEX)

	if err != nil {
		return nil, errors.New("Unable to get AllPatients")
	}

	var allPatients AllPatients

	err = json.Unmarshal(bytes, &allPatients)

	if err != nil {
		return nil, errors.New("Corrupt AllPatients record")
	}
	...
	if err != nil {
		fmt.Print("Error creating AllPatients record")
	}
...
func (t *SimpleChaincode) update_vin(stub shim.ChaincodeStubInterface, v Vehicle, caller string, caller_affiliation string, new_value string) ([]byte, error) {
	...
	if err != nil {
		fmt.Printf("UPDATE_VIN: Error saving changes: %s", err); return nil, errors.New("Error saving changes")
	}
